<?php
	...
	function searchArticles($arguments){
		
		try{
			if(!isset($arguments['searchTextInput']) ||  !isset($arguments['searchType'])){
				die("Rellene el campos de \"texto a buscar\" y elija donde buscar ");
			...
			if (!$GLOBALS["conn"]) {
				...
				$date=null;
				$type=null;
				$fullSql='SELECT * FROM `article` WHERE (`tags`="opinion" OR `tags`="cultura" OR `tags`="deportes" OR `tags`="politica" OR`tags`="entrevistas") AND (`date`="") AND (`text`="")'; 
				$sql="SELECT * FROM `article` ORDER BY `article`.`date` DESC";	
				
				
				$proceduralSql="SELECT * FROM `article` WHERE";
				$isSection=false;
				$isDate=false;
				
				if(isset($arguments['searchSectionCU'])){

					array_push($sectionsArray,$arguments['searchSectionCU']);
					$isSection=true;
				}
				if(isset($arguments['searchSectionOP'])){
					array_push($sectionsArray,$arguments['searchSectionOP']);
					$isSection=true;
				}
				if(isset($arguments['searchSectionDE'])){
					array_push($sectionsArray,$arguments['searchSectionDE']);
					$isSection=true;
				}
				if(isset($arguments['searchSectionPO'])){
					array_push($sectionsArray,$arguments['searchSectionPO']);
					$isSection=true;
				}
				if(isset($arguments['searchSectionEN'])){
					array_push($sectionsArray,$arguments['searchSectionEN']);
					$isSection=true;
				}
				if($isSection){
					$proceduralSql.="(";
					 for ($i = 0; $i < count($sectionsArray); $i++) {
						$proceduralSql.="(`tags`='". $sectionsArray[$i]."')" ;
						if (isset($sectionsArray[$i + 1])){
							$proceduralSql.="OR ";
						}
				...
				/*if(isset($arguments['searchDate'])){
					$date=$arguments['searchDate'];
					if($isSection){
						$proceduralSql.=" AND ";
					}
					$proceduralSql.=" (`date`=".$date.") ";
					//echo $date;
				}*/
				if($isSection ){
					$proceduralSql.=" AND ";
				...
				if($arguments['searchType']="0"){
					$proceduralSql.=" (`text` LIKE '%".$arguments['searchTextInput'] ."'%)";
				}else{
					$proceduralSql.=" (`title` LIKE '%".$arguments['searchTextInput'] ."%')";
				}
				
				if ($stmt =  $GLOBALS["conn"]->prepare($proceduralSql)) {	
					//echo $proceduralSql;
