	...
	if(!isset($_POST['functionname']) ) {
		...
		switch($_POST['functionname']) {
			...
			case 'getAllAds':
				 ...
				 getAllAds();				
			break;
			case 'searchArticlesTitles':
								
				 searchArticlesTitles($_POST['arguments']);				
			break;
			case 'searchAdText':
								
				 searchAdText($_POST['arguments']);				
			break;
		...
		try{
			...
			if(empty($_POST['articleTitle']) || empty($_POST['articleAutor']) || empty($_POST['articleDate']) ||
				...
				if (!$GLOBALS["conn"]) {
					...
					$title=cleanText($_POST['articleTitle']);
					$text=cleanText($_POST['articleText']);
					...
					if ($stmt =  $GLOBALS["conn"]->prepare($sql)) {
						...
						$stmt->bind_param("sssssss",$_POST['articleDate'],$title,$text,$name,$imgDesc,$_POST['articleAutor'],$_POST['articleTag']);	
						if($allOK){
							$stmt->execute();
		...
		try{
			if (!$GLOBALS["conn"]) {
					...
					$sql="SELECT * FROM `advert`
						WHERE `text` LIKE "."'%".$words[0]."%'";
							
					for($i = 1; $i < $max;$i++)
					{
						$append=" OR `text` LIKE "."'%".$words[$i]."%'";
